...
if __name__ == '__main__':
    ...
    t1, rules = apriori(transactions, minSup=0.029, minConf=0.1)  # apriori_python
    ...
    print()
    start = time.perf_counter()
    rules2 = apriori2(transactions, min_support=0.022, min_confidence=0.1, min_lift=1.0001)  # apyori
    ...
    start = time.perf_counter()
    items, rules3 = apriori3(transactions, min_support=0.028, min_confidence=0.1)
    ...
    print()
    start = time.perf_counter()
    items, rules4 = fpgrowth(transactions, minSupRatio=0.03, minConf=0.19)
